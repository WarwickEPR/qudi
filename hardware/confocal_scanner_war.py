...
class ConfocalScannerWarwick(Base, ConfocalScannerInterface):
    ...
    def on_activate(self, e):
        ...
        if 'war_scanner_com_port' in config.keys():
            self._wscanner_com_port = config['war_scanner_com_port']
        else:
            self.log.error('No parameter "war_scanner_com_port" found in config.\n'
                            'Cannot connect to confocal scanner controller!')

        if 'war_scanner_baud_rate' in config.keys():
            self._wscanner_baud_rate = config['war_scanner_baud_rate']
        else:
            self._wscanner_baud_rate = 9600
            self.log.warning('No parameter "war_scanner_baud_rate" found in config!\n'
                    'Taking the baud rate {0} instead.'.format(self._wscanner_baud_rate))

        self._wscanner_timeout = 10000

        self._serial_connection_wscanner = serial.Serial(port=self._wscanner_com_port,
                                                         baudrate=self._wscanner_baud_rate,
                                                         bytesize=8,
                                                         parity='N',
                                                         stopbits=1,
                                                         timeout=self._wscanner_timeout)
        self._serial_connection_wscanner.write(b'N?\r\n')
        dx = self._serial_connection_wscanner.readline()

        self.log.info('The number of points on the warwick scanner are: {0}.'.format(dx))
...
#       gaussian_function(self,x_data=None,amplitude=None, x_zero=None, sigma=None, offset=None):
    ...
    def reset_hardware(self):
        ...
        self._serial_connection_wscanner.close()
        return 0
    ...
    def set_position_range(self, myrange=None):
        ...
        @return int: error code (0:OK, -1:error)
        """
        self.log.warning('Setting position range')
    ...
    def set_voltage_range(self, myrange=None):
        ...
        @return int: error code (0:OK, -1:error)
        """
        self.log.warning('Setting the voltage range of scanner')
        if myrange is None:
            myrange = [0.,10.]
    ...
    def set_up_scanner(self, counter_channels=None, sources=None,
        ...
        self.log.warning('Values are: {0}, {1}, {2}, {3}'.format(counter_channels, sources, clock_channel, scanner_ao_channels))
    ...
    def scanner_set_position(self, x=None, y=None, z=None, a=None):
        ...
        self.log.warning('Going to ({0},{1},{2})'.format(x,y,z))
        if self.getState() == 'locked':
