...
class AttocubeController(Base, MotorInterface):
    ...
    def _enable_DC_input(self, name):
        """ Enables or disables the DC input for the given axis """
        self._send_cmd("setdci {} on".format(self._axis_config[name]['id']))

    def _disable_DC_input(self, name):
        """ Enables or disables the DC input for the given axis """
        self._send_cmd("setdci {} off".format(self._axis_config[name]['id']))
