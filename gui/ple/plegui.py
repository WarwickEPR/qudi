# -*- coding: utf-8 -*-
"""
This file contains the Qudi GUI module to perform a PLE experiment
...
class PLEMainWindow(QtWidgets.QMainWindow):
    def __init__(self):
        # Get the path to the *.ui file
        this_dir = os.path.dirname(__file__)
        ui_file = os.path.join(this_dir, 'ui_plegui.ui')

        # Load it
        super(PLEMainWindow, self).__init__()
...
class PLEGui(GUIBase):
    ...
    ## declare connectors
    plelogic = Connector(interface='PLELogic')
    ...
    def on_activate(self):
        ...
        self.scan_matrix_image = pg.ImageItem(
            self._plelogic.scan_matrix,
            axisOrder='row-major')

        self.scan_matrix_image.setRect(
            QtCore.QRectF(
                self._plelogic.scan_range[0],
                0,
                self._plelogic.scan_range[1] - self._plelogic.scan_range[0],
                self._plelogic.number_of_repeats)
        )

        self.scan_matrix_image2 = pg.ImageItem(
            self._plelogic.scan_matrix2,
            axisOrder='row-major')
        ...
        self.scan_image = pg.PlotDataItem(
            self._plelogic.plot_x,
            self._plelogic.plot_y)

        self.scan_image2 = pg.PlotDataItem(
            self._plelogic.plot_x,
            self._plelogic.plot_y2)

        self.scan_fit_image = pg.PlotDataItem(
            self._plelogic.fit_x,
            self._plelogic.fit_y,
...
#        self._mw.speedDoubleSpinBox.setValue(self._plelogic._scan_speed)
        ...
        self._mw.linesSpinBox.setValue(self._plelogic.number_of_repeats)
        ...
        # Connect signals
        self._plelogic.sigUpdatePlots.connect(self.refresh_matrix)
        self._plelogic.sigUpdatePlots.connect(self.refresh_plot)
        self._plelogic.sigUpdatePlots.connect(self.refresh_lines)
        self._plelogic.sigScanFinished.connect(self.scan_stopped)
        self._plelogic.sigScanStarted.connect(self.scan_started)
...
#        self.sigChangeSpeed.connect(self._plelogic.set_scan_speed)
    ...
    def update_wavelength_controls(self):
        ...
        if self._plelogic._tunable_laser.get_wavelength_control_mode() is WavelengthControlMode.WAVELENGTH_IN_METERS:
            self._mw.startDoubleSpinBox.setSuffix('m')
            self._mw.stopDoubleSpinBox.setSuffix('m')
            self._mw.constDoubleSpinBox.setSuffix('m')
        else:
            self._mw.startDoubleSpinBox.setSuffix('V')
            self._mw.stopDoubleSpinBox.setSuffix('V')
            self._mw.constDoubleSpinBox.setSuffix('V')
    ...
    def refresh_plot(self):
        ...
        self.scan_image.setData(self._plelogic.plot_x, self._plelogic.plot_y)
        self.scan_image2.setData(self._plelogic.plot_x, self._plelogic.plot_y2)

    def refresh_matrix(self):
        """ Refresh the xy-matrix image """
        self.scan_matrix_image.setImage(self._plelogic.scan_matrix, axisOrder='row-major')
        self.scan_matrix_image.setRect(
            QtCore.QRectF(
                self._plelogic.scan_range[0],
                0,
        ...
        self.scan_matrix_image2.setImage(self._plelogic.scan_matrix2, axisOrder='row-major')
        self.scan_matrix_image2.setRect(
            QtCore.QRectF(
                self._plelogic.scan_range[0],
                0,
        ...
        scan_image_data = self._plelogic.scan_matrix
        ...
        scan_image_data2 = self._plelogic.scan_matrix2
        # Now update image with new color scale, and update colorbar
